...
class FAService:
    ...
    def load_required_data(self) -> Tuple[
        ...
        with Progress() as progress:
            ...
            logger.info(f"Auto-discovered {len(gl_files)} G&L statement files for Foreign Assets analysis")
            ...
            if exchange_rate_files:
                ...
                logger.info(f"Using exchange rate file for FA: {primary_sbi_file.name}")
                
                try:
                    sbi_loader = SBIRatesLoader(primary_sbi_file)
                    sbi_records = sbi_loader.get_validated_records()
                    logger.info(f"✅ Loaded {len(sbi_records)} SBI exchange rate records for FA")
                except Exception as e:
            ...
            if adobe_stock_files:
                ...
                logger.info(f"Using Adobe stock file for FA: {primary_stock_file.name}")
                
                try:
                    stock_loader = AdobeStockDataLoader(primary_stock_file)
                    stock_records = stock_loader.get_validated_records()
                    logger.info(f"✅ Loaded {len(stock_records)} Adobe stock price records for FA")
                except Exception as e:
    ...
    def calculate_fa_for_year(
        ...
        results = FACalculationResults(
            ...
            year_summaries={str(calendar_year): fa_summary},
            total_years_analyzed=1,
            years_requiring_declaration=[str(calendar_year)] if fa_summary.declaration_required else []
        )
